...
interface IPoolForLens is IPool {
    ...
    function assetsInfo(address _token) external view returns (AssetInfo memory);
    ...
    function getTrancheValue(bool _max) external view returns (uint256 sum);
    function averageShortPrices(address _token) external view returns (uint256);
...
/*contract PoolL ens {
    ...
    function poolAssets(address _pool, address _token) external view returns (PoolAsset memory poolAsset) {
        ...
        for (uint256 i = 0; i < nTranches;) {
            ...
            uint256 shortSize = self.assetsInfo(tranche, _token).totalShortSize;
    ...
    function getTrancheValue(IPoolForLens _pool, address _tranche) external view returns (uint256) {
        return (_pool.getTrancheValue(true) + _pool.getTrancheValue(false)) / 2;
    }
    ...
    function getAssetAum(address _poolAddress, address _token, bool _max) external view returns (uint256) {
        ...
        (uint256 poolAmount, uint256 reservedAmount, uint256 guaranteedValue, uint256 totalShortSize) =
            _pool.assetsInfo(_token);
        if (isStable) {
            return poolAmount * price;
        } else {
            uint256 averageShortPrice = _pool.averageShortPrices(_token);
    ...
    function getAssetPoolAum(IPoolForLens _pool, address _token, bool _max) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < nTranches; ++i) {
            ...
            AssetInfo memory asset = _pool.assetsInfo(_token);
            if (isStable) {
