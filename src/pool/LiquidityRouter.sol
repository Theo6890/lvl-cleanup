...
contract LiquidityRouter {
    ...
    function addLiquidityETH(uint256 _minLpAmount, address _to) external payable {
        uint256 amountIn = msg.value;
        weth.deposit{value: amountIn}();
        weth.safeIncreaseAllowance(address(pool), amountIn);
        pool.addLiquidity(address(weth), amountIn, _minLpAmount, _to);
    }
    ...
    function addLiquidity(address _token, uint256 _amountIn, uint256 _minLpAmount, address _to) external {
        ...
        pool.addLiquidity( _token, _amountIn, _minLpAmount, _to);
    }

    function removeLiquidityETH(uint256 _lpAmount, uint256 _minOut, address payable _to) external {
        IERC20 lpToken = IERC20(plpToken);
    ...
    function removeLiquidity(address _tokenOut, uint256 _lpAmount, uint256 _minOut, address _to) external {
        ...
        lpToken.safeTransferFrom(msg.sender, address(this), _lpAmount);
        lpToken.safeIncreaseAllowance(address(pool), _lpAmount);
        pool.removeLiquidity( _tokenOut, _lpAmount, _minOut, _to);
    }
